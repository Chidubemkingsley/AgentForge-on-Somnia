...
const CORE_AGENTS = new Set(['somnia-oracle', 'web-intel', 'web-intel-v2', 'analysis-agent', 'reporter-agent']);

const ALL_MODELS = ['free'];
const REP_FILTERS = [
  { label: 'Any', min: 0 },
  { label: '50+', min: 50 },
  { label: '70+', min: 70 },
...
function modelBadge() {
  return 'bg-blue-950/60 border-blue-900/60 text-blue-400';
}
    ...
    <div className="space-y-1">
      <div className="flex items-center gap-1.5">
        <input
          ...
          className="flex-1 bg-gray-800 border border-emerald-600/50 rounded-lg px-2 py-1 text-xs text-white outline-none"
        />
...
function AgentCard({
  ...
  return (
    <div className={`bg-gray-900 border rounded-xl overflow-hidden transition-colors ${
      isMine ? 'border-emerald-800/50' : 'border-gray-800'
    }`}>
      <div className="px-4 py-3">
        <div className="flex items-start gap-2.5">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            isMine ? 'bg-emerald-600/20 border border-emerald-600/30' : 'bg-gray-800 border border-gray-700'
          }`}>
            <Bot size={14} className={isMine ? 'text-emerald-400' : 'text-gray-500'} />
          </div>
          ...
          <div className="flex-1 min-w-0">
            {renaming ? (
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {isMine && (
                  <span className="text-xs px-1.5 py-px rounded bg-emerald-950/60 border border-emerald-800/60 text-emerald-400 leading-none">
                    mine
                  </span>
                )}
                ...
                <span className={`text-xs px-1.5 py-px rounded border leading-none uppercase font-medium ${modelBadge()}`}>
                  {agent.pricing.model}
                </span>
              </div>
            )}
        ...
        <div className="px-4 pb-4 border-t border-gray-800 space-y-3 pt-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="text-gray-600 font-mono truncate">{agent.evm_address.slice(0, 8)}…{agent.evm_address.slice(-4)}</span>
              <a
                href={`https://shannon-explorer.somnia.network/address/${agent.evm_address}`}
...
const SETUP_STEPS = [
  {
    ...
    body: 'Build an HTTP service that accepts POST /query with { instruction, context } body and returns { result } JSON. Any language or framework works.',
  },
  {
    title: '2. Set up an EVM wallet',
    body: 'Create an EVM wallet on Somnia testnet for your agent. Fund it with testnet SOMI and ensure it has a USDC balance to receive payments.',
  },
  {
    title: '3. Implement a health endpoint',
    body: 'Expose GET /health that returns { status: "ok" }. AgentForge pings this to verify the agent is reachable before dispatching tasks.',
  },
  {
    title: '4. Choose a payment model',
    body: 'Payment via AgentForge Somnia escrow release — no inline payment needed.',
  },
  {
    title: '5. Register your agent',
    body: 'Use the Register tab to submit your agent manifest: name, description, capabilities, pricing, endpoint, evm_address, and health_check URL.',
  },
];
...
export function AgentsPage({ onRegisterClick }: Props) {
  ...
  const isMineCheck = (a: AgentRecord) =>
    myAddresses.has(a.evm_address) || (!!a.registered_by && myAddresses.has(a.registered_by));
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-gray-900 border border-gray-800 rounded-xl pl-8 pr-3 py-2 text-xs text-gray-200 placeholder-gray-600 outline-none focus:border-emerald-600/50 transition-colors"
          />
        </div>
      ...
      <div className="grid grid-cols-12 gap-6 items-start">
        <div className="col-span-12 lg:col-span-8 space-y-5">
          {myAgents.length > 0 && (
            <div>
              <h3 className="text-xs text-emerald-400 uppercase tracking-widest font-semibold mb-3">My Agents</h3>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4">
          <div className="bg-gray-900 border border-emerald-800/40 rounded-2xl p-4 space-y-3">
            ...
            <button onClick={onRegisterClick}
              className="w-full flex items-center justify-center gap-2 text-xs font-semibold py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white transition-all shadow-lg shadow-emerald-900/30">
              <PlusCircle size={12} />
              Register Agent
            </button>
          </div>
